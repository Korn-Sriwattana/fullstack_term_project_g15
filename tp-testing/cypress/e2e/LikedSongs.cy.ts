...
describe("Liked Songs Page - Logged In", () => {
  ...
  const initialLikedSongs = [
    {
      id: "like-1",
      likedAt: new Date("2024-01-15").toISOString(),
      song: {
        id: "s-1",
        title: "First Liked Song",
        artist: "Artist One",
        duration: 210,
    ...
    {
      id: "like-2",
      likedAt: new Date("2024-02-20").toISOString(),
      song: {
        id: "s-2",
        title: "Second Liked Song",
        artist: "Artist Two",
        duration: 180,
        coverUrl: "https://via.placeholder.com/50",
      },
    ...
    {
      id: "like-3",
      likedAt: new Date("2024-03-10").toISOString(),
      song: {
        id: "s-3",
        title: "Third Liked Song",
        artist: "Artist Three",
        duration: 240,
        coverUrl: "https://via.placeholder.com/50",
      },
  ...
  const stubSession = () => {
    ...
    cy.intercept("GET", "**/api/current-user**", {
      statusCode: 200,
      body: { user: USER, source: "mock" },
    }).as("currentUser");
    cy.intercept("GET", "**/api/me", { statusCode: 200, body: { user: null } });
    cy.intercept("GET", "**/api/user/check**", {
      statusCode: 200,
      body: { exists: true, user: USER },
    }).as("userCheck");
  };
  ...
  it("should display all liked songs", () => {
    ...
    cy.contains(`${mockLikedSongs.length} songs`)
      .scrollIntoView()
      .should("be.visible");
  });
  ...
  it("should remove a song when heart button is clicked", () => {
    ...
    cy.get('[class*="resultItem"]').should(
      "have.length",
      initialLikedSongs.length - 1
    );
  ...
  it("should show empty state when all songs are removed", () => {
    ...
    cy.contains("You haven't liked any songs yet", { timeout: 10000 }).should(
      "be.visible"
    );
    cy.contains("Tap the heart on tracks you love", { timeout: 10000 }).should(
      "be.visible"
    );
  });
});
