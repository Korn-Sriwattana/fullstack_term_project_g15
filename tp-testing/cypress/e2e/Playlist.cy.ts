...
describe("Playlist Page - Logged In ", () => {
  ...
  const stubPlaylistsApi = () => {
    ...
    cy.intercept("GET", `${API_URL}/playlists/*/songs**`, (req) => {
      ...
      const playlistIdIndex =
        urlParts.findIndex((part) => part === "playlists") + 1;
  ...
  it("สามารถสร้าง playlist ใหม่", () => {
    ...
    cy.get('textarea[placeholder="Describe your playlist..."]').type(
      "Test Description"
    );
    cy.get("select").select("public");

    cy.get('[class*="modalActions"]')
  ...
  it("สามารถเปิด playlist detail view และแสดง 'No songs in this playlist' เมื่อไม่มีเพลง", () => {
    ...
    cy.contains("No songs in this playlist")
      .scrollIntoView()
      .should("be.visible");
  });
  ...
  it("แสดงว่า your playlist is empty เมื่อไม่มี playlist", () => {
    cy.intercept("GET", `${API_URL}/playlists/${USER.id}?mode=owner`, {
      statusCode: 200,
      body: [],
    }).as("getEmptyPlaylists");
    cy.reload();
